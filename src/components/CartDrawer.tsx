...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const [paymentMethod, setPaymentMethod] = useState<'cod' | 'vietqr' | 'card'>('vietqr');
  const [orderNote, setOrderNote] = useState('Giao vào giờ hành chính, gọi trước khi đến');

  const subtotal = items.reduce((acc, item) => acc + item.product.price * item.quantity, 0);
  const discountAmount = Math.round((subtotal * discountPercent) / 100);
  const shippingFee = subtotal >= 150000 || subtotal === 0 ? 0 : 25000;
                          ...
                          {items.map((item, idx) => (
                            ...
                            >
                              <img
                                ...
                                className="w-15 h-15 object-cover rounded-lg bg-stone-100 shrink-0 border border-stone-200"
                              />
                              <div className="flex-1 min-w-0">
                                <h5 className="font-sans text-xs font-bold text-stone-900 truncate">
                                  {item.product.name}
                                </h5>
                                <div className="flex flex-wrap gap-1 text-[11px] text-stone-500 my-1 font-medium">
                                  {item.size && (
                                    <span className="bg-white border border-stone-200 px-1.5 py-0.2 rounded text-[10px]">
                                      {item.size}
                                    </span>
                                  )}
                                  {item.grindOption && (
              ...
              <div className="p-5 border-t border-stone-200 bg-white space-y-3 shrink-0">
                <div className="space-y-1.5 text-xs text-stone-600 font-medium">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-stone-800">{shippingFee === 0 ? 'Miễn phí' : `${shippingFee.toLocaleString('vi-VN')}đ`}</span>
                  </div>
